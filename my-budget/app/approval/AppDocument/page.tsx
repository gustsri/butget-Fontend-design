...
import mockApprovalData from "@/data/mockApprovalData.json";

export default function ApprovalPage() {
    const currentYear = new Date().getFullYear() + 543;

    // หาปีทั้งหมดจาก mock
    const years = [...new Set(mockApprovalData.map((doc) => Number(doc.year)))];
    ...
    // ✅ filter: ถ้าเลือก "ทั้งหมด" → แสดงทุกปี
    const filteredDocuments =
        selectedYear && selectedYear !== "ทั้งหมด"
            ? mockApprovalData.filter((doc) => Number(doc.year) === selectedYear)
            : mockApprovalData;
                ...
                <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
                    ...
                    <div className="bg-gradient-to-r from-blue-800 to-blue-900 text-white p-6">
                        <h1 className="text-2xl font-bold text-center">
                            เอกสารจัดทำแผนที่สามารถอนุมัติได้
                        </h1>
                        <p className="text-center mt-2 text-blue-100">
                            จัดการและอนุมัติแผนงบประมาณรายรับ-รายจ่าย
                        </p>
                    </div>
